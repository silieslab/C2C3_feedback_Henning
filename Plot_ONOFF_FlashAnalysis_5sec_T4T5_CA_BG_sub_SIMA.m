...
% New with background subtraction 
clc;
...
ANALYZE_BY_REGION = false;% weather ROIs should be selected based on their layer in the lobula complex. 
REGION = 'Layer1';  % which group or layer of ROIs will be included in this plot
% AT=axon terminals; AX=axons; CB=cell bodies; DE=dendrites
% this region info is stored as a string in the variable 'layer'in the MASTER excel spreadsheet

if(ANALYZE_BY_REGION)
    %GENOTYPE_STRING = ['C2 shits >> GCaMP6f, region: ' REGION];
     GENOTYPE_STRING = ['T4T5rec >> GCaMP6f, region: ' REGION];

    if(strcmp(REGION,'Layer1'))
        COLOR_OF_PLOT = [0 .5 0];% GREEN
        COLOR_CLOUD =[.5 .7 .5];% GREEN
    elseif(strcmp(REGION,'Layer3'))
        COLOR_OF_PLOT = [1 0 0]; %RED
        COLOR_CLOUD = [1 .5 .5]; %RED cloud
    elseif(strcmp(REGION,'Layer4'))
        COLOR_OF_PLOT = [.7 .7 0]; % YELLOW
        COLOR_CLOUD = [.9 .9 .5]; %YELLOW cloud
    elseif(strcmp(REGION,'Layer2')) 
        COLOR_OF_PLOT = [0 0 1]; %BLUE; 
        COLOR_CLOUD = [.5 .5 1]; %blue cloud
    end
else
    GENOTYPE_STRING = 'T4T5rec>> GCaMP6f';
    COLOR_OF_PLOT = [0 0 1]; %BLUE; [.7 .7 0]% YELLOW %[1 0 0]; %RED% [0 .5 0]% GREEN %[0 0 1]; %BLUE;
    %COLOR_OF_PLOT = [1 0 0]; %RED; %[0 0 1] %BLUE;
    COLOR_CLOUD = [.5 .5 1];
end
...
FFFDATA_T4T5_C2Sil=load('FFFDATA_T4T5_Control.mat');
FFFDATA_T4T5_C3Sil=load('FFFDATA_T4T5_Control.mat');
FFFDATA_T4T5_C2C3Sil=load('FFFDATA_T4T5_Control.mat');


% mTm9LexA = aggregate_fffall_means10Hz_BleedThruFix(xTm9LexA);
mCsplitGal4 = aggregate_fffall_means10Hz_BleedThruFix_v2_CA(xCsplitGal4);
mCsplitGal4_C2Sil = aggregate_fffall_means10Hz_BleedThruFix_v2_CA(xCsplitGal4_C2Sil);
mCsplitGal4_C3Sil = aggregate_fffall_means10Hz_BleedThruFix_v2_CA(xCsplitGal4_C3Sil);
mCsplitGal4_C2C3Sil = aggregate_fffall_means10Hz_BleedThruFix_v2_CA(xCsplitGal4_C2C3Sil);
...
% % cur_mat = cur_mat(inds,:);
% % cur_IDs = cur_IDs(inds);
% % % mean_res = mean(cur_mat,1); %was correlating with the mean beforee
% % % Q = corr(mean_res',cur_mat');

cur_t = [1:size(cur_mat_T4,2)]/iRATE;

%positive correlation with stimulus
% % Q = corr(mean(mCsplitGal4.stims)',cur_mat_T4');
% % % Q = corr(mean(mTm9LexA.stims)',cur_mat');
% %  inds = find(Q>0); %finds the cells whose response correlates well with the stimulus 
% % %inds = find(Q>0.1); %ONLY for ort!!!!!
% % cur_mat_pos = cur_mat_T4(inds,:); 
...
% % 
% Q = corr(mean(mCsplitGal4.stims)',cur_mat_T5_C3Sil');
%  inds = find(Q<-0.5);
% %inds = find(Q<-0.1); %ONLY for the L2 ort control!!!!!
% size(inds)
% cur_mat_neg = cur_mat(inds,:);%% FOR NOW neg corr. cells include all!!!!!!
...
%% Figure1: Plot mean +std

F1=figure; hold on
set(F1,'Position', [ 400 400 1500 600])
t = [1:dur]/10;
...
subplot(1,2,2)
h4 = plot_err_patch_v2(cur_t,m_T5_C2Sil,e_T5_C2Sil,[0.55 0.4 0.55],[0.55 0.5 0.55]); %Green for C3  
...
L=legend([h1,h3,h5],sprintf(['T4 Control- N( %d )'],size(x_T4,1)),...
    sprintf(['T4 C2Silenced- N( %d )'],size(x_T4_C2Sil,1)),...
...
L=legend([h2,h4,h6],sprintf(['T5 Control- N( %d )'],size(x_T5,1)),...
    sprintf(['T5 C2Silenced- N( %d )'],size(x_T5_C2Sil,1)),...
...
L=legend([h1,h3,h5,h7],sprintf(['T4 Control- N( %d )'],size(x_T4,1)),...
    sprintf(['T4 C2Silenced- N( %d )'],size(x_T4_C2Sil,1)),...
    sprintf(['T4 C3Silenced- N( %d )'],size(x_T4_C3Sil,1)),...
...
L=legend([h2,h4,h6,h8],sprintf(['T5 Control- N( %d )'],size(x_T5,1)),...
    sprintf(['T5 C2Silenced- N( %d )'],size(x_T5_C2Sil,1)),...
    sprintf(['T5 C3Silenced- N( %d )'],size(x_T5_C3Sil,1)),...
...
%% Figure1: Plot mean +std seperated Layers 

F4=figure; hold on
set(F4,'Position', [ 400 400 1500 800])
t = [1:dur]/10;
% subplot(2,2,1); hold on;
cm=colormap('lines');
subplot(4,2,1)
...
% & Increase of OFF response for T5 

T4_ONResps=nan(4,10);
T5_OFFResps=nan(4,10); 
T5_ONResps=nan(4,10); 

T4_ON_Delay=nan(4,10);
...
T4_ON_Delay(3,1:length(MAX))=delay/iRATE;
...
F3=figure; hold on
set(F3,'Position', [ 400 400 1500 600])
...
title('ON response - T4','FontSize', 18) 
ylabel('change of dF/F','FontSize', 18);
set(gca,'FontSize',12)
...
set(box1, 'Color', '[0.1 0.5 1]'); set(box2, 'Color', '[0 0.5 0]');  set(box3, 'Color', '[0.75 0 0.75]');  set(box4, 'Color', '[0.7 0.7 0.7]'); 
set(Median1, 'Color', 'k');  set(Median2, 'Color', 'k');  set(Median3, 'Color', 'k');  set(Median4, 'Color', 'k');   % 
title('ON response - T5','FontSize', 18) 
ylabel('change of dF/F','FontSize', 18);
set(gca,'FontSize',12)
...
boxplot(T4_ON_Delay', 'notch', 'on','labels', {'Control', 'C2 Silence', 'C3 Silence', 'C2C3 Silence'} )
...
set(box1, 'Color', '[0.1 0.5 1]'); set(box2, 'Color', '[0 0.5 0]');  set(box3, 'Color', '[0.75 0 0.75]');  set(box4, 'Color', '[0.7 0.7 0.7]'); 
set(Median1, 'Color', 'k');  set(Median2, 'Color', 'k');  set(Median3, 'Color', 'k');  set(Median4, 'Color', 'k');   % 
title('time to peak (s) - T4','FontSize', 18) 
ylabel('change of dF/F','FontSize', 18);
set(gca,'FontSize',12)
...
% %% Two-sample Kolmogorov-Smironov test
% 
% [h,p]=kstest(T4_ONResps(1,1:end-1)) %(end-1) to remove NaNs from test statistics
% [h,p]=kstest(T4_ONResps(2,1:end-1))
% [h,p]=kstest(T4_ONResps(3,:))
% [h,p]=kstest(T5_OFFResps(1,1:end-3))
% [h,p]=kstest(T5_OFFResps(2,1:end-3))
% [h,p]=kstest(T5_OFFResps(3,:))
% 
% [h,p]=lillietest(T4_ONResps(1,1:end-1))
% 
% 
% [p,h]=ranksum(T4_ONResps(1,:),T4_ONResps(3,:))
% [p,h]=ranksum(T4_ONResps(1,:),T4_ONResps(2,:))
% [p,h]=ranksum(T4_ONResps(3,:),T4_ONResps(2,:))
% 
% 
% [p,h]=ranksum(T5_OFFResps(1,:),T5_OFFResps(2,:))
% [p,h]=ranksum(T5_OFFResps(1,:),T5_OFFResps(3,:))
% [p,h]=ranksum(T5_OFFResps(2,:),T5_OFFResps(3,:))
% 
% [p,h]=ranksum(T4_ON_Delay(1,:),T4_ON_Delay(2,:))
% [p,h]=ranksum(T4_ON_Delay(1,:),T4_ON_Delay(3,:))
% [p,h]=ranksum(T4_ON_Delay(2,:),T4_ON_Delay(3,:))
% 
