...
function [Timing,timeEl,timeAz]=Plot_timeSectionSTRFC2C3_gh(RF_DATA, NCON,Predicition_thres2,Col)
...
%     title('Timing of highest correlation')
    ...
    figure
    subplot(2,1,1)
    plot_err_patch_v2(t, mean([timeAv_Az],2),std([timeAv_Az],[],2)/sqrt(size([timeAv_Az],2)),Col,Col);
    ...
    subplot(2,1,2)
    plot_err_patch_v2(t, mean([timeAv_El],2),std([timeAv_El],[],2)/sqrt(size([timeAv_El],2)),Col,Col);
    line([t(1) t(end)],[0 0],'color',[0 0 0],'LineStyle','--');
    title(['- T5'])
    set(gca, 'FontSize', 12)
    set(gca, 'XTick', [-2, -1,  0])
