...
for NFlies= 1:length(T4T5_mb)
     ...
     Ang=angle(Z_T5A); 
     VL_T5A_mean(NFlies)=mean(abs(Z_T5A));
%      ng=find(Ang)<=0; 
...
%      Ang(ng)=Ang(ng)+2*pi; %Convert from -pi->pi scale to 0->2pi scale 
     ...
     VL_T5B_mean(NFlies)=mean(abs(Z_T5B));
...
%      Ang(ng)=Ang(ng)+2*pi; %Convert from -pi->pi scale to 0->2pi scale 
     ...
     VL_T5C_mean(NFlies)=mean(abs(Z_T5C));
...
%      Ang(ng)=Ang(ng)+2*pi; %Convert from -pi->pi scale to 0->2pi scale 
     ...
     VL_T5D_mean(NFlies)=mean(abs(Z_T5D));

     
     
     
     VL_T5_mean(NFlies)=mean(abs([Z_T5A,Z_T5B,Z_T5C,Z_T5D]));
...
%      Ang(ng)=Ang(ng)+2*pi; %Convert from -pi->pi scale to 0->2pi scale 
     ...
     VL_T4A_mean(NFlies)=mean(abs(Z_T4A));
...
%      Ang(ng)=Ang(ng)+2*pi; %Convert from -pi->pi scale to 0->2pi scale 
     ...
     VL_T4B_mean(NFlies)=mean(abs(Z_T4B));
...
%      Ang(ng)=Ang(ng)+2*pi; %Convert from -pi->pi scale to 0->2pi scale 
     ...
     VL_T4C_mean(NFlies)=mean(abs(Z_T4C));
...
%      Ang(ng)=Ang(ng)+2*pi; %Convert from -pi->pi scale to 0->2pi scale 
     ...
     VL_T4D_mean(NFlies)=mean(abs(Z_T4D));
     
     
     
     VL_T4_mean(NFlies)=mean(abs([Z_T4A,Z_T4B,Z_T4C,Z_T4D]));
    ...
    Z.T4A.VL_M=VL_T4A_mean; %Vector length averaged across flies
    ...
    Z.T5D.VL_M=VL_T5D_mean;
    
    Z.T5.VL_M=VL_T5_mean;
    Z.T4.VL_M=VL_T4_mean;
